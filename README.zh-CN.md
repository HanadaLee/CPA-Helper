...
已发布教程展示在 **API 密钥** 页的密钥列表和 Endpoint 下方。管理员可在 **系统设置 → 教程管理** 中维护标题（例如 `Codex Windows Desktop`）、排序、草稿/发布状态、Markdown 正文和可选英文翻译，并预览效果。每篇教程以标题作为独立标签，只展示一篇文章；不再设置分类、软件或操作系统字段，也不合并文章。各语言下的标题不允许重名，草稿和英文留空回退到中文的情况也参与校验。升级为已有重名标题补编号，不删除文章，保留正文和发布设置，并为原始 Codex CLI 教程补上 Windows、macOS、Linux 标题以便区分。教程只填充一次，后续编辑或删除不会被重启覆盖。

在正文或代码块中，可通过编辑器工具栏插入以下变量：

- `{{api_key}}`：读者自己已启用的 API 密钥。
- `{{model_id}}`：来自 **可用模型** 的实际模型 ID，而非显示名称。
- `{{api_base_url}}`（别名 `{{api_endpoint}}`）：包含 `/v1` 的基础 URL。
- `{{responses_url}}`、`{{chat_completions_url}}`、`{{claude_messages_url}}`：对应类型的请求 URL。

点击变量即可复制：只有一个可用选项时直接复制，多个选项时弹出选择框，选项较多时支持搜索。点击 **复制代码** 会按需选择密钥、Endpoint 和模型，并替换整段代码中的变量。模型按需从 **可用模型** 的同一接口加载，刷新 API 密钥页会清除页面内的模型缓存；同时复制密钥和模型时，只提供所选密钥可用的模型。已禁用或没有完整值的密钥不参与选择。真实密钥不会存入教程内容，也不会自动显示在文章中。管理员编辑的教程按可信内容处理，预览和正式文章均支持标准 Markdown 图片及原生 HTML，不额外限制图片域名或格式；图片会按文章容器宽度等比缩放。
