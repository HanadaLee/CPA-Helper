...
点击变量即可复制：只有一个可用选项时直接复制，多个选项时弹出选择框，选项较多时支持搜索。点击 **复制代码** 会按需选择密钥、Endpoint 和模型，并替换整段代码中的变量。模型按需从 **可用模型** 的同一接口加载，刷新 API 密钥页会清除页面内的模型缓存；同时复制密钥和模型时，只提供所选密钥可用的模型。已禁用或没有完整值的密钥不参与选择。真实密钥不会存入教程内容，也不会自动显示在文章中。教程不渲染原始 HTML 或远程图片。
