...
已发布教程展示在 **API 密钥** 页的密钥列表和 Endpoint 下方。管理员可在 **系统设置 → 教程管理** 中维护单一分类名称（例如 `Codex Windows Desktop`）、排序、草稿/发布状态、Markdown 正文和可选英文翻译，并预览效果。每篇教程独立保存，不再按软件和操作系统分别筛选。旧教程的客户端与平台会自动合并为 `Codex CLI Windows` 等分类，正文和发布设置保持不变。迁移会一次性填充 Windows、macOS、Linux 的 Codex CLI 教程，后续编辑或删除不会被重启覆盖。
